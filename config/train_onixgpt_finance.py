# OnixGPT finance model config (small, Apple Silicon friendly)
# Edit model size, block_size, and training steps for your hardware and data

out_dir = 'out-onixgpt-finance'  # Output directory for checkpoints
...
# We expect to overfit on small datasets, so only save when val improves
eval_iters = 200

# Weights & Biases logging (optional)
wandb_log = False
wandb_project = 'onixgpt-finance'
wandb_run_name = 'mini-onixgpt'

dataset = 'finance_notes'  # Looks for data/finance_notes.txt

# Training hyperparameters
# Adjust batch_size and block_size for your GPU/CPU memory
batch_size = 32  # Lower if you run out of memory
block_size = 128  # Context window (tokens)

# Model size (small for MacBook Air)
n_layer = 4  # Number of transformer layers (increase for bigger models)
